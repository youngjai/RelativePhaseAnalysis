...
%% t-test
[h_ec_mode, p_ec_mode] = ttest2(sub_mode_C_ec, sub_mode_Ain_ec);
[h_eo_mode, p_eo_mode] = ttest2(sub_mode_C_eo, sub_mode_Ain_eo);
[h_ec_dwell, p_ec_dwell] = ttest2(sub_dwell_C_ec, sub_dwell_Ain_ec);
[h_eo_dwell, p_eo_dwell] = ttest2(sub_dwell_C_eo, sub_dwell_Ain_eo);
...
mean_mode = cellfun(@(x) mean(x, 'all', 'omitnan'), data_mode).*100;
...
for i = 1:16  
    ...
    % Run daviolinplot
    h = daviolinplot(data_mode{i} .* 100, 'violin', 'full', ...
...
y_bracket = [75 58 58 80];        % Y-axis position for the bracket
...
line([x_bracket(5), x_bracket(6)], [y_bracket(3), y_bracket(3)], 'Color', 'k', 'LineWidth', 1.5);  
line([x_bracket(5), x_bracket(5)], [y_bracket(3), y_bracket(3)-2], 'Color', 'k', 'LineWidth', 1.5); 
line([x_bracket(6), x_bracket(6)], [y_bracket(3), y_bracket(3)-2], 'Color', 'k', 'LineWidth', 1.5); 

line([x_bracket(7), x_bracket(8)], [y_bracket(4), y_bracket(4)], 'Color', 'k', 'LineWidth', 1.5);  
line([x_bracket(7), x_bracket(7)], [y_bracket(4), y_bracket(4)-2], 'Color', 'k', 'LineWidth', 1.5);
line([x_bracket(8), x_bracket(8)], [y_bracket(4), y_bracket(4)-2], 'Color', 'k', 'LineWidth', 1.5); 
...
for i = 1:length(shading_intervals)
    ...
    patch(x_patch, y_patch, [1, 0.95, 0.46], 'FaceAlpha', 0.2, 'EdgeColor', 'none');  
end
