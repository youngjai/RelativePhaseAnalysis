function dataOut = topoplot_general_temp(temp_Z, Coord, varargin) 
...
%
%   multi:          Toggles using parfor for multithreaded processing.
%                   Default is 1 (on).
%
%   bin_mat         NxN sized matrix of 1 and 0 denoting the connection
%                   matrix of input nodes.
%
%   topo            Toggles the plotting of the topoplot. Default is 1;
%
%   edge_thick      Sets the thickness of edges on network. Default is 1
%
%   edge_color      Sets the color of edges on the network. Default is 'k'
%
%   w_mat           NxN weighted matrix
%
%   thresh          Sets the threshold for including edge in the network
%
% Feb 2, 2016 by Joseph Lee
% Last updated: Jan 26, 2017 by Joseph Lee
...
defaultMulti = 1;
p.addParameter('multi', defaultMulti, @isnumeric);
defaultbin = [0];
p.addParameter('bin_mat', defaultbin, @isnumeric);
defaultTopo = 1;
p.addParameter('topo', defaultTopo, @isnumeric);
defaultEdgeThick = 1;
p.addParameter('edge_thick', defaultEdgeThick, @isnumeric);
defaultEdgeColor = 'k';
p.addParameter('edge_color', defaultEdgeColor, @isalpha);
defaultbin = [0];
p.addParameter('w_mat', defaultbin, @isnumeric);
defaultThresh = 0;
p.addParameter('thresh', defaultThresh, @isnumeric);
...
multiFlag = p.Results.multi;
bin_mat = p.Results.bin_mat;
topoFlag = p.Results.topo;
edge_thick = p.Results.edge_thick;
edge_color = p.Results.edge_color;
w_mat = p.Results.w_mat;
...
elseif shiftpreset == 14
    ...
    shift = [-25 25 -50 50];
    load('mouseBorderInner.mat')
elseif shiftpreset == 15
    %rat
    shift = [-125 125 -100 100];
    load('ratBorderInner.mat')
end
%parse color
if color_arg == 0 
...
if(multiFlag == 1)
    z_trans = reduce_resolution_circle_multithread(z,resolutionRadius,0.2);
else
...
%draw network edges
if (size(bin_mat,1) ~= 1 && size(bin_mat,2) ~= 1)
    hold on;
    for i = 1:L
        for j = i:L
            if(bin_mat(i,j) > 0)
                plot([Coord(i,1) Coord(j,1)], [Coord(i,2) Coord(j,2)], edge_color, 'LineWidth', edge_thick)
            end
        end
    end
elseif (size(w_mat,1) ~= 1 && size(w_mat,2) ~= 1)
    range_min = thresh;
    range_max = max(max(w_mat));
    hold on;
    for i = 1:L
        for j = i:L
            if(w_mat(i,j) > thresh)
                edge_thick = ((w_mat(i,j) - range_min) / (range_max - range_min)) * (3 - 0.5) + 0.5;
                plot([Coord(i,1) Coord(j,1)], [Coord(i,2) Coord(j,2)], edge_color, 'LineWidth', edge_thick)
            end
        end
    end
