function dataOut = topoplot_general(temp_Z, Coord, borderCoords, varargin) 
...
%                   9 = monkey theory, 10 = 96ch ketamine, 11 = human side simulation
...
p.parse(temp_Z, Coord, borderCoords, varargin{:});
...
elseif shiftpreset == 11
    %human simulation to match channel plot
    shift = [-125 125 -25 100];
    load('humanSideBorder.mat');
end
%parse color
if color_arg == 0 
...
z_trans = reduce_resolution_circle(z,resolutionRadius,0.2);
dataOut = z_trans;
%graph
contourf(xx,yy,z_trans); 
